 ...
 * overdue rent and the coming week's maintenance schedule. Recipients are
 * active owner/admin/finance/manager users, individually opt-out-able via the
 * per-user notification toggles (Settings → Notifications). Cron-only (never
 * runs at worker startup) so restarts cannot double-send.
 */
