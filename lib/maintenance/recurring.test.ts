...
describe("periodKeyFor", () => {
  it("returns the YYYY-MM of the property-timezone civil month", () => {
    expect(periodKeyFor(new Date("2026-06-15T12:00:00-04:00"), "America/New_York")).toBe(
      "2026-06",
    );
  });
  ...
  it("uses the property timezone's civil month, not the UTC month", () => {
    ...
    expect(periodKeyFor(instant, "America/Los_Angeles")).toBe("2026-06");
    expect(periodKeyFor(instant, "UTC")).toBe("2026-07");
  });
  ...
  it("rolls the month forward at the local boundary in an ahead-of-UTC tz", () => {
    ...
    expect(periodKeyFor(instant, "Australia/Sydney")).toBe("2026-06");
    expect(periodKeyFor(instant, "UTC")).toBe("2026-05");
  });

  it("zero-pads single-digit months", () => {
    expect(periodKeyFor(new Date("2026-01-09T12:00:00Z"), "UTC")).toBe("2026-01");
  });
});
